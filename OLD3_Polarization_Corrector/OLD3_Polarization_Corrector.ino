...
const float pVolThr = 0.6; // Threshold (in volts) for deciding to correct the waveplate orientation
...
void UpdatePlot(float zoom, float val1, float val2, float val3) {
  ...
  Serial.println(zoom*val3); // Plot val3
}
...
void setup() {
  ...
  Serial.println("Initializing half-waveplate controller..."); // This is being sent via USB to programming computer, not to the SMC100
  SMC100Begin(1); // Connect to the SMC100 and prepare it to receive orders
  Serial.println("DONE");
  Serial.println("Initializing quarter-waveplate controller...");
  SMC100Begin(2);
  Serial.println("DONE");
...
void loop() { 
  UpdatePhotodiodeAverage(analogRead(pPinLo), analogRead(pPinHi)); // Update our photodiode values
  UpdatePlot(10, pVolOut, pVolAvg, float(state)); // Plot zoomed in graph with latest photodetector voltage, its rolling average and the state. Zoom is set to an arbitrary number (10 right now).
  ...
  if (state == 0) { // If system does not need correcting
    ...
    if (readyToRotate) {
      ...
      if (rotatePositive) {
        Rotate(2, 1.0*stepAngle); // Rotate the waveplate in the positive direction
      }
      else {
        Rotate(2, -1.0*stepAngle); // Rotate the waveplate in the negative direction
      }
    }
    ...
    else if (pVolAvg < 0.8*pVolThr) { // (NOTE: the 0.9 is an arbitrary choice) If we are back under the threshold (and a little further than that still)...
